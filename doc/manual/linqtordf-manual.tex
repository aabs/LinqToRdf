...
\texttt{System.Linq} which will be released as part of Visual Studio .NET 2008.
...
Mono yet, but since Mono 1.9 now supports \cs{} it should be able to run.
...
a \texttt{LinqToRdf-0.x} directory under ``
...
\texttt{mp3s.n3} file. In mp3s.n3 I defined a namespace called ns1. It refers to what
...
as they were called in \texttt{music.n3}.  It doesn’t matter what you call the prefix
for the namespace, just so long as the URI that it maps to is the same as was
used in the ontology definition file. I called it \texttt{ns1}, because that’s what my
import program wanted to do. The point is that the type \texttt{ns1:Track} in this file
refers to the \texttt{:Track} class defined in \texttt{music.n3}. The triple store that we’ll get
...
now) be either Jena (\texttt{jena.sf.net}) or OpenLink
(\texttt{openlinksw.com}), though there are various triple store solutions that
...
tested with the Joseki \spq{} interface to Jena, and the OpenLink Virtuoso
platform. LinqToRdf is platform independent in the sense that it will happily
work on any standards compliant \spq{} server.
...
bit to get right is to use the same URI in \texttt{OntologyAttribute} as we used in
\texttt{music.n3} and \texttt{mp3s.n3} for the namespace definitions. Using
\texttt{OntologyAttribute}
plus \texttt{OntologyName} allows you to define all attributes as relative URIs which
makes for a much more readable source file. The \texttt{OwlResourceAttribute} defines
our .NET class ‘\texttt{RdfMusic.Track}’ to correspond with the OWL class
\texttt{http://aabs.purl.org/ontologies/2007/04/music\#Track}. Likewise the
‘\texttt{FileLocation}’ property defined on it corresponds to the RDF datatype property
\texttt{http://aabs.purl.org/ontologies/2007/04/music\#fileLocation}. The Boolean true on
...
simple LINQ enabled console application called \texttt{MyRdfTest}. Open up
\texttt{Program.cs} up
for editing, and add namespace import statements for \texttt{System.Linq},
\texttt{LinqToRdf} and \texttt{SemWeb}:
...
In \texttt{Main}, create a \texttt{TripleStore} object with the location of the \spq{} server:
...
\texttt{TripleStore} is used to carry any information needed about the triple store for
later use by the query. In this case I set up an IIS virtual directory on my
local machine called \texttt{linqtordf}, and followed the steps outlined early. The
\texttt{QueryType} just indicates to the query context that we will be using \spq{} over
...
interested in  the \texttt{Title} and the \texttt{FileLocation}.
...
you’re only interested in getting some \texttt{Track}s back! In the references section
...
\texttt{LinqToRdfDesigner}. This section describes the tasks required to set up a very
simple ontology of two classes called \texttt{Artifact} and \texttt{Assembly}. The program
...
file extension for LinqToRdfDesigner files is ‘\texttt{rdfx}’. Click on the
‘\textsf{New Item}’
...
Change the name of the file to \texttt{SystemScannerModel.rdfx}. This will create
...
\texttt{SystemScannerModel.rdfx} is the design surface file.
\texttt{SystemScannerModel.rdfx.entities.tt} is a text template file that is used to
generate the \cs{} file containing the base definitions of your .NET entity model.
\texttt{SystemScannerModel.rdfx.n3.tt} is the text template that is used to generate the
...
By default, it will be called something like \texttt{ModelClass1}. Rename it to
‘\texttt{Artifact}’.
...
‘\texttt{Artifact}’ in the ‘\textsf{Owl Class Uri}’ property in the property window.
...
‘\texttt{ArtifactExists}’. Again, use the properties window to set up some properties
...
What we’re saying here is that the .NET property ‘\texttt{ArtifactExists}’ of .NET type
\texttt{bool} corresponds to the OWL URI ‘\texttt{artifactExists}’ which LinqToRdf will convert
into the XML Schema Datatype ‘\texttt{xsdt:boolean}’. You can ignore the description
...
the text templates to generate documentation for both \cs{} and N3.
...
‘Artifact’. This will connect the classes using direct inheritance in \cs{} and
\texttt{owl:subclass} in N3. Your domain model is now taking shape. 
...
\texttt{ArtifactStore} is a wrapper around a SemWeb \texttt{MemoryStore}.
...
In LinqToRdf you can use the \texttt{StmtObjectWithSubjectAndPredicate} and
\texttt{StmtSubjectWithObjectAndPredicate} to
instruct it to navigate the relationships using \spq{} syntax. here is an
example of \texttt{StmtObjectWithSubjectAndPredicate} in action:
...
Given an object called \texttt{album} whose instance URI we already know, we can
select through properties that we know reference that URI. The definition of
property \texttt{Album} in class \texttt{Track} looks like this:
...
The combination of \texttt{t}, \texttt{Album} and \texttt{StmtSubjectWithObjectAndPredicate} is enough
...
Which will filter all tracks except those that are tracks on \texttt{ns1:Album\_2}.
...
\texttt{InstanceUri} property, so you will have to construct your collections
manually.
...
\texttt{DataContext} and \texttt{InstanceUri} of \texttt{this}. It will store the
query and only invoke it on first demand. You need to beware that the
\texttt{DataContext} has not gone out of scope before the query gets invoked. That
